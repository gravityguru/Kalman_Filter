...
def sim_run(options, KalmanFilter):
    ...
    time_a = []
    noise = []
    dt = float(input("Enter sampling time : "))
    time_len = int(100) #int(input("Enter time lenngth : "))
    pts = int(time_len/dt+1)
    ...
    for t0 in t:
        ...
        time_a.append(t0)
        v_est_a.append(v_est_data)
        est_data_t += [t0]
            # Measure car location.
        
        noise = np.array(np.random.rand(1)[0]-0.5)*0.3
        # noise.append(noise)
    ...
    time_a = arr.reshape(pts,1)
    print(time_a)
    
    print("-------------")
    state_a = np.array(state_a[0])
    x = state_a.reshape(pts,1)
    x = x/dt
    v_est_a = np.array(v_est_a[0])
    v_est_a = v_est_a.reshape(pts,1)
    # noise = np.array(noise[0])
    # noise = noise.reshape(pts,1)
    print((state_a))
    print("-------------")
    print((v_est_a))
    print("-------------")
    print((noise))
    plt.plot(time_a, v_est_a, '--r')
    plt.plot(time_a, x,'--b')
    ...
    plt.xticks([])
    ax3.set_ylim(-4,4)
    ax3.set_yticks([-4,-3,-2,0,1,2,3,4])
