...
def sim_run(options, KalmanFilter):
    ...
    def physics(t0,dt,state):
        ...
        if x0 < 75 and t0 < 30:
            u_pedal = 5
            u_steer = 0
        elif t0 < 30: # First Red signal time
            u_pedal = 0
            u_steer = 0
        elif theta0 < 1.45:
            u_pedal = 5
    ...
    # Main plot info.
    car, = ax.plot([], [], 'r-', linewidth = 10)
    light, = ax.plot([94,94], [4,2] , 'r-', linewidth = 3)
    est, = ax.plot([], [], 'bs', markersize=20, fillstyle='none', linewidth=10)
    meas, = ax.plot([], [], 'gs', markersize=30, fillstyle='none', linewidth=10)
